...
export default function CheckoutClient({ initialCustomerInfo }: CheckoutClientProps) {
  ...
  const [formData, setFormData] = useState({
    fullName: initialCustomerInfo?.fullName || "",
    email: initialCustomerInfo?.email || "",
    contactHandle: initialCustomerInfo?.phone || "",
    notes: "",
  ...
  // Synchronize or fallback-fetch customer info if client mounted without server prefill
  useEffect(() => {
    if (!initialCustomerInfo) {
      getCustomerCheckoutInfoAction()
        .then((res) => {
          if (res.success && res.data) {
            ...
            setFormData((prev) => ({
              ...
              contactHandle: prev.contactHandle || data.phone || ""
            }));
          }
        })
        .catch(() => {});
  ...
  return (
    <div className="min-h-screen bg-background-subtle/50 py-20 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-[1100px] mx-auto">
        
        {/* Progress Steps Tracker */}
        <div className="relative mb-14 max-w-xl mx-auto px-4">
          <div className="absolute top-1/2 left-4 right-4 h-[3px] bg-slate-200/80 -translate-y-1/2 rounded-full -z-10" />
          <motion.div 
            className="absolute top-1/2 left-4 h-[3px] bg-primary -translate-y-1/2 rounded-full -z-10" 
              ...
              return (
                <div key={s.id} className="flex flex-col items-center select-none">
                  <button
                    ...
                    disabled={s.id >= step}
                    className={`
                      relative flex items-center justify-center w-12 h-12 rounded-2xl border-2 transition-all duration-300 shadow-sm cursor-pointer disabled:cursor-default overflow-hidden
                      ${isCurrent 
                        ? 'border-primary text-white scale-105 ring-4 ring-primary/15 shadow-md shadow-primary/20' 
                        : isActive 
                          ? 'bg-white border-primary text-primary hover:bg-slate-50' 
                          : 'bg-white border-slate-200/80 text-slate-300'
                      }
                    `}
                  >
                    {isCurrent && (
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-slate-500 mt-2 text-xs max-w-md mx-auto font-normal">
            ...
            {step === 2 && (dict?.checkout?.pageDesc2 || (locale === "vi" ? "Hệ thống sẽ tự động sử dụng thông tin tài khoản của bạn để gửi key bản quyền và link tải mã nguồn." : "Our system will use your account details to dispatch the license key and download link."))}
          ...
          >
            ...
            {step === 1 && (
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                ...
                <div className="lg:col-span-2 bg-white border border-slate-100 rounded-3xl p-6 md:p-8 shadow-xs">
                  <h2 className="text-base font-semibold text-slate-900 mb-6 flex items-center gap-2.5 tracking-tight">
                        ...
                        <div className="flex-1 flex flex-col justify-between">
                          <div>
                            ...
                            <div className="flex flex-wrap gap-1.5 mt-1.5">
                              ...
                              <span className="text-xs text-slate-500 font-normal bg-slate-100 px-2 py-0.5 rounded font-mono">
                          ...
                          <div className="flex justify-between items-end mt-2">
                            ...
                            <p className="font-bold text-slate-900 text-base font-mono">{formatCurrency(item.price, locale)}</p>
                          </div>
                        </div>
                      </div>
                    ))}
                ...
                <div className="lg:col-span-1">
                  <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-xs sticky top-24">
                    <h2 className="text-base font-semibold text-slate-900 mb-6 tracking-tight">
                    ...
                    <div className="space-y-4 border-b border-slate-100 pb-6 mb-6">
                      <div className="flex justify-between text-xs text-slate-500 font-normal">
                        ...
                        <span className="text-slate-900 font-medium font-mono">{formatCurrency(subtotal, locale)}</span>
                      </div>
                      <div className="flex justify-between text-xs text-slate-500 font-normal">
                    ...
                    <button 
                      ...
                      className="w-full py-3 bg-primary hover:bg-primary-dark active:scale-98 text-white rounded-xl font-medium flex items-center justify-center gap-2 group transition-all shadow-xs cursor-pointer text-sm"
                    >
            ...
            {step === 2 && (
              <div className="max-w-2xl mx-auto bg-white border border-slate-100 rounded-3xl p-6 md:p-10 shadow-xs">
                <div className="mb-6">
                ...
                {(formData.fullName || formData.email || formData.contactHandle) && (
                  <div className="mb-6 flex items-center gap-2.5 px-3.5 py-2.5 bg-emerald-50/80 border border-emerald-200/60 rounded-xl text-xs text-emerald-800 font-medium">
                    <Sparkles className="w-4 h-4 text-emerald-600 shrink-0" />
                    <span>
                      {locale === "vi" 
                        ? "Hệ thống đã tự động điền thông tin từ tài khoản của bạn. Bạn vẫn có thể chỉnh sửa nếu cần." 
                        : "Information has been automatically filled from your profile. You can still modify it if needed."}
                    </span>
                  </div>
                )}
                ...
                <div className="space-y-5">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className={`w-full h-10 pl-10 pr-4 bg-white border rounded-xl text-sm font-normal focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all ${
                          validationErrors.fullName ? 'border-red-400 bg-red-50/20' : 'border-slate-200/80'
                        }`}
                      />
                    </div>
                    {validationErrors.fullName && (
                      <p className="text-red-500 text-xs mt-1 font-normal">{validationErrors.fullName}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className={`w-full h-10 pl-10 pr-4 bg-white border rounded-xl text-sm font-normal focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all ${
                          validationErrors.email ? 'border-red-400 bg-red-50/20' : 'border-slate-200/80'
                        }`}
                      />
                    </div>
                    {validationErrors.email && (
                      <p className="text-red-500 text-xs mt-1 font-normal">{validationErrors.email}</p>
                    )}
                  </div>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-700 mb-1.5">
                      {dict?.checkout?.contactHandleLabel || (locale === "vi" ? "Kênh liên hệ hỗ trợ (Số điện thoại / Zalo / Facebook)" : "Support Contact Handle (Phone / Zalo / Facebook)")} <span className="text-red-500">*</span>
                    </label>
                    <div className="relative">
                      ...
                      <input 
                        ...
                        onChange={handleChange}
                        placeholder={dict?.checkout?.contactHandlePlaceholder || (locale === "vi" ? "Số điện thoại, Zalo hoặc link Facebook cá nhân" : "Phone number, Zalo or Telegram handle")}
                        className={`w-full h-10 pl-10 pr-4 bg-white border rounded-xl text-sm font-normal focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all ${
                          validationErrors.contactHandle ? 'border-red-400 bg-red-50/20' : 'border-slate-200/80'
                        }`}
                      />
                    </div>
                    {validationErrors.contactHandle && (
                      <p className="text-red-500 text-xs mt-1 font-normal">{validationErrors.contactHandle}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <textarea 
                      ...
                      className="w-full p-3.5 bg-white border border-slate-200/80 rounded-xl text-sm font-normal focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all resize-none"
                    />
                  </div>
                </div>
                ...
                <div className="flex gap-4 mt-8">
                  <button 
                    onClick={() => setStep(1)}
                    className="w-1/3 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-medium flex items-center justify-center gap-2 transition-all cursor-pointer text-sm active:scale-95"
                  >
                  ...
                  <button 
                    ...
                    className="w-2/3 py-2.5 bg-primary hover:bg-primary-dark active:scale-98 text-white rounded-xl font-medium flex items-center justify-center gap-2 group transition-all shadow-xs cursor-pointer text-sm"
                  >
            ...
            {step === 3 && (
              <div className="max-w-2xl mx-auto bg-white border border-slate-100 rounded-3xl p-6 md:p-10 shadow-xs space-y-6">
                <div>
                ...
                <div className="bg-slate-50 border border-slate-100 rounded-2xl p-5 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-xs">
                    <div>
                      <span className="text-slate-400 block font-normal">{dict?.checkout?.fullNameLabel || "Name"}:</span>
                      <span className="font-medium text-slate-900">{formData.fullName}</span>
                    </div>
                    <div>
                      <span className="text-slate-400 block font-normal">{dict?.checkout?.emailLabel || "Email"}:</span>
                      <span className="font-medium text-slate-900 font-mono">{formData.email}</span>
                    </div>
                    <div>
                      <span className="text-slate-400 block font-normal">{dict?.checkout?.contactHandleLabel || "Contact"}:</span>
                      <span className="font-medium text-slate-900 font-mono">{formData.contactHandle}</span>
                    </div>
                    {formData.notes && (
                      <div>
                        <span className="text-slate-400 block font-normal">{dict?.checkout?.notesLabel || "Notes"}:</span>
                        <span className="font-normal text-slate-600 italic line-clamp-1">{formData.notes}</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="flex gap-4 pt-2">
                  <button 
                    onClick={() => setStep(2)}
                    disabled={loading}
                    className="w-1/3 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-medium flex items-center justify-center gap-2 transition-all cursor-pointer text-sm disabled:opacity-50 active:scale-95"
                  >
                    <ArrowLeft className="w-4 h-4" />
                    <span>{dict?.checkout?.backToCart || (locale === "vi" ? "Quay lại" : "Back")}</span>
                  </button>
                  <button 
                    ...
                    disabled={loading}
                    className="w-2/3 py-3 bg-primary hover:bg-primary-dark active:scale-98 text-white rounded-xl font-medium flex items-center justify-center gap-2 transition-all shadow-xs cursor-pointer text-sm disabled:opacity-70 disabled:cursor-not-allowed"
                  >
                    {loading ? (
                      <>
